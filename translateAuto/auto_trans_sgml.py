...
async def process_tag(tag,commenttag,ctx):
    ...
    if tag.name in CN_TAGS:
        ...
        fdest.write(ctx.src_content[ctx.pos:insert_pos])
        fdest.write(ctx.src_content[insert_pos:tag.start])
        content = ctx.comment_content[commenttag.start:commenttag.end]
        poeBot.setClipboard(content)
        content_cn = await poeBot.translate()
        fdest.write(content_cn)
        # fdest.write(content)
        ctx.pos = commenttag.end
    else:
        srctags = tag.childs()
        ...
        for i in range(0,len(srctags)):
            await process_tag(srctags[i],commenttags[i],ctx)
...
def process_file(sourceFile,commentFile,destFile):
    ...
    if(len(srctags) != len(commenttags)):
        raise Exception('child tag number of tag "%s" in source sgml (%d) and comment sgml (%d) does not match' % (tag.name,len(srctags),len(commenttags)))
    for i in range(0,len(srctags)):
        print('process_tag 1111111111')
        # process_tag(srctags[i],commenttags[i],commentTagContext)
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop = asyncio.get_event_loop()
        loop.run_until_complete(process_tag(srctags[i],commenttags[i],commentTagContext))
        loop.close()
...
if __name__ == "__main__":
    ...
    asyncio.run(poeBot.getLightDBDocBoy())
